...
class Ad_Rotate_Email_Addon_Send_Email {
    ...
    public function send_email($id,$month) {
        ...
        $x=0;
        $adstats1 = array();
        $adstats1['clicks'] = $adstats1['impressions'] = 0;
        if( !empty( $ads ) ){
            foreach($ads as $ad) {
                // Prevent gaps in display
                if( $ad['impressions'] == 0 || $ad['impressions'] == '' ) $ad['impressions'] = 0;
                if( $ad['clicks'] == 0 || $ad['clicks'] == '' ) $ad['clicks'] = 0;
            ...
            if ($adstats) {
               foreach ($adstats as $stat) {
                ...
                if(isset($useremail)) {
                    ...
                    $html = '<div class="error">';
                    $html .= '<p>';
                    $html .= __( 'Please Add user to Ad for Send mail', 'adrotate-email-add-on' );
                    $html .= '</p>';
                    $html .= '</div><!-- /.updated -->';
                    echo $html;
                }
